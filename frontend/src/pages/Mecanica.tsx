...
export default function Mecanica() {
  ...
  async function doLogin(number: string, pin: string) {
    ...
    try {
      ...
      if (data.tech.team && data.tech.team !== team) {
        alert("Este técnico não tem acesso à Mecânica.");
        return;
      }
  ...
  if (!authed) {
    return (
      <DashboardLayout title="Mecânica">
    ...
    >
      ...
      <div className="card card-pad mecanica-list" style={{ marginTop: 12 }}>
        ...
        <div className="grid" style={{ marginTop: 12 }}>
          ...
          {sorted.length === 0 && <div className="p">Sem avarias mecânicas por tratar.</div>}
        </div>
      </div>
      </div>
    </DashboardLayout>
